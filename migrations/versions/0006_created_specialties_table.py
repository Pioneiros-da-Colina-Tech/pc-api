...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_specialties_id'), 'specialties', ['id'], unique=True)
    op.create_table('user_specialties',
    sa.Column('user_id', app.infra.database.entities.GUID(), nullable=False),
    sa.Column('specialty_id', app.infra.database.entities.GUID(), nullable=False),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=True),
    sa.Column('deleted_at', sa.TIMESTAMP(timezone=True), nullable=True),
    sa.Column('id', app.infra.database.entities.GUID(), nullable=False),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'specialty_id', name='uq_user_specialty')
    )
    op.create_index(op.f('ix_user_specialties_id'), 'user_specialties', ['id'], unique=True)
